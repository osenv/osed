# Template — Deadline/Duty-Suit Consent Decree Scaffold (Negotiated Resolution of a Citizen-Suit Failure-to-Act Case)

> **DRAFT SCAFFOLD — NOT AN AGREEMENT, NOT FOR LODGING OR ENTRY.** This is a statute-agnostic *scaffold* for the negotiated resolution of a citizen-suit failure-to-act case (e.g., a Clean Water Act §505(a)(2) / 33 U.S.C. §1365(a)(2) deadline suit). A consent decree is a **negotiated agreement** between the parties **and** a court order. This scaffold **proposes no terms** — every substantive term (the compliance-schedule dates, the fees, the no-admission framing, modification/force-majeure, dispute resolution, termination) is for the parties and their counsel to negotiate; the software proposes none of them. It is **not** an agreement until the parties agree, and **not effective** until it is lodged, published for public comment, and entered by the court. This template is a starting structure, not legal advice. See `../DISCLAIMER.md`.

## Why the form matters

A consent decree is both a **contract** between the parties and a **court order**: it binds both parties and is enforceable by the court that enters it. Because it is a negotiated agreement, the agent supplies **structure only** — every negotiated term is the parties' to set, and the software proposes none of them. And because it is a court order, the court does not rubber-stamp it: it enters the decree only **after the public-comment process** and a review for **fairness, reasonableness, and consistency with the statute** the suit was brought under. Until the court enters it, the decree binds no one.

> [⚠ ATTORNEY: every term in this scaffold is a negotiated term — the parties and their counsel set the schedule, fees, no-admission framing, and all other terms; the software proposes none. Confirm the applicable lodging/comment/entry procedure (e.g., 28 C.F.R. § 50.7); the decree is not effective until the court enters it.]

## Required elements checklist

Build and verify this before drafting prose. Each must be present or carry an attorney flag. No term below is supplied by the software — every substantive term is a `[placeholder]` the parties negotiate.

- [ ] **Caption** — same court and parties as the complaint this decree settles `[⚠ ATTORNEY: confirm court and parties]`
- [ ] **WHEREAS recitals** — the pending suit; the alleged nondiscretionary duty; the parties' intent to resolve the case **without admission of law, fact, or liability** `[⚠ ATTORNEY: the no-admission framing is a negotiated term — the parties set it; the software does not propose it]`
- [ ] **Definitions** — terms used in the decree `[⚠ ATTORNEY: negotiated term]`
- [ ] **The court's jurisdiction** and its authority to enter the decree
- [ ] **Compliance schedule** — the agreed date(s) by which the duty is to be performed `[placeholder]` `[⚠ ATTORNEY: negotiated term — the parties set the schedule; the software does not propose dates]`
- [ ] **No-admission-of-liability** clause `[⚠ ATTORNEY: negotiated term — the parties set the no-admission framing; the software does not propose it]`
- [ ] **Reporting / progress obligations** `[⚠ ATTORNEY: negotiated term]`
- [ ] **Modification / good-cause extension / force-majeure** `[⚠ ATTORNEY: negotiated term]`
- [ ] **Dispute-resolution procedure** `[⚠ ATTORNEY: negotiated term]`
- [ ] **Retention of jurisdiction** by the court to enforce the decree
- [ ] **Costs and attorney/expert fees** `[placeholder]` `[⚠ ATTORNEY: negotiated term]`
- [ ] **Lodging → public comment → entry** clause `[⚠ ATTORNEY: confirm the comment/entry procedure (e.g., 28 C.F.R. § 50.7); not effective until entered]`
- [ ] **Effective date / termination** `[⚠ ATTORNEY: negotiated term]`
- [ ] **Signatures** of all parties and counsel; a `SO ORDERED` court-entry line

---

## DRAFT

```
IN THE UNITED STATES DISTRICT COURT FOR THE
[⚠ ATTORNEY: district] [⚠ ATTORNEY: confirm the proper court — same court and division as the complaint]

------------------------------------------------------------------
[placeholder — full legal name of plaintiff],

                 Plaintiff,

       v.                                  Civil Action No. [placeholder]

[placeholder — full legal name of defendant
(e.g., the agency officer sued in their official capacity)],

                 Defendant.
------------------------------------------------------------------

                  [PROPOSED] CONSENT DECREE

[⚠ ATTORNEY: caption must match the complaint — confirm court and parties.]

WHEREAS, plaintiff commenced this action under the underlying
citizen-suit statute [⚠ ATTORNEY: cite the statute and subsection of
the underlying suit — e.g., CWA § 505(a)(2), 33 U.S.C. § 1365(a)(2)],
alleging that defendant failed to perform a nondiscretionary duty by a
statutory deadline [⚠ ATTORNEY: confirm the statute and the duty from
the complaint — this scaffold is statute-agnostic; the specific
authorities are filled from the underlying suit];

WHEREAS, the parties wish to resolve this action without further
litigation, and **without any admission of law, fact, or liability** by
any party [⚠ ATTORNEY: the no-admission framing is a negotiated term —
the parties set it; the software does not propose it];

WHEREAS, the parties have negotiated the terms set out below, each of
which is a term the parties agreed to and none of which is proposed by
the software [⚠ ATTORNEY: every term below is a negotiated term];

NOW, THEREFORE, it is STIPULATED by the parties, and upon entry by the
Court ORDERED, ADJUDGED, and DECREED as follows:

1.  JURISDICTION

    The Court has jurisdiction over this action and over the parties,
    and the authority to enter this decree. [⚠ ATTORNEY: confirm the
    jurisdictional basis from the complaint (e.g., 28 U.S.C. § 1331 and
    CWA § 505(a) / 33 U.S.C. § 1365(a)).]

2.  DEFINITIONS

    [placeholder — defined terms used in this Decree, e.g., "the Plan,"
    "Day," "Effective Date."] [⚠ ATTORNEY: the defined terms are a
    negotiated drafting choice; the software proposes none.]

3.  COMPLIANCE SCHEDULE

    Defendant shall perform the duty described in the recitals by
    [placeholder — the agreed compliance date(s)]. [⚠ ATTORNEY:
    negotiated term — the parties set the schedule; the software does
    not propose dates.]

4.  NO ADMISSION

    [placeholder — the no-admission-of-liability provision]. This decree
    resolves the action without any admission of law, fact, or liability
    by any party. [⚠ ATTORNEY: negotiated term — the parties set the
    no-admission framing; the software does not propose it.]

5.  REPORTING

    [placeholder — defendant's reporting and progress obligations].
    [⚠ ATTORNEY: negotiated term.]

6.  MODIFICATION / FORCE MAJEURE

    [placeholder — the modification, good-cause-extension, and
    force-majeure provisions]. [⚠ ATTORNEY: negotiated term.]

7.  DISPUTE RESOLUTION

    [placeholder — the dispute-resolution procedure]. [⚠ ATTORNEY:
    negotiated term.]

8.  COSTS AND FEES

    [placeholder — the provision for costs and attorney/expert fees].
    [⚠ ATTORNEY: negotiated term — the parties set any fee amount; the
    software does not propose a figure.]

9.  PUBLIC COMMENT AND ENTRY

    This decree shall be lodged with the Court and published for public
    comment before entry [placeholder — the agreed lodging-and-comment
    procedure]. The decree is **not effective until the Court enters it**,
    and the Court enters it only after reviewing it for fairness,
    reasonableness, and consistency with the statute. [⚠ ATTORNEY:
    confirm the comment/entry procedure (e.g., 28 C.F.R. § 50.7 — lodge
    at least 30 days before entry, publish for comment, file comments
    with the court, and the United States may withdraw consent); the
    decree is not effective until entered.]

10. RETENTION OF JURISDICTION

    The Court retains jurisdiction over this action to enforce the terms
    of this decree. [⚠ ATTORNEY: confirm the scope and duration of
    retained jurisdiction — this is a negotiated term.]

11. EFFECTIVE DATE / TERMINATION

    [placeholder — the effective-date and termination provisions]. This
    decree takes effect upon entry by the Court. [⚠ ATTORNEY: negotiated
    term — the parties set the effective-date and termination terms; the
    software does not propose them.]

AGREED:

[placeholder — counsel for plaintiff: name, bar number, firm, address,
phone, email]

[placeholder — counsel for defendant: name, bar number, office/agency,
address, phone, email]

[⚠ ATTORNEY: signature area — all parties and their counsel must review
and sign; the software supplies the blocks, not the signatures.]

SO ORDERED this [placeholder — date of entry].

_________________________________
United States District Judge

[⚠ ATTORNEY: the court enters the decree; do not present it as entered.]
```

---

## Negotiated-terms & comment/entry note

This scaffold proposes no term and is not effective until lodged, published for comment, and entered by the court. This software tracks no clock.

## Consolidated attorney flags (carry forward into the Drafting agent's output)

- **Every term is negotiated** — the parties and their counsel set the compliance schedule, fees, definitions, reporting, modification/force-majeure, dispute resolution, effective date, and termination; the software proposes none of them.
- **No-admission framing** — the resolution without admission of law, fact, or liability is a negotiated term the parties set.
- **Retention of jurisdiction** — confirm the scope and duration of the court's retained jurisdiction to enforce the decree (a negotiated term).
- **Comment/entry procedure** — confirm the applicable lodging/comment/entry procedure (e.g., 28 C.F.R. § 50.7); the decree is not effective until the court enters it.
- **Court's review** — the court enters the decree only after reviewing it for fairness, reasonableness, and consistency with the statute the suit was brought under.
- **Counsel review and signature** — all parties and their counsel must review and sign before the decree is presented to the court.
- **Court entry** — the court enters the decree; do not present it as agreed or entered.
