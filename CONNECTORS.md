...
> **Status:** Built — see [`connectors/regulatory/`](connectors/regulatory/). Six tools — four keyless: `get_uscode_section` (GovInfo US Code), `get_current_regulation` (eCFR), `find_agency_actions` and `find_rule_changes` (Federal Register; the latter is the rule-change half of the currency check) — and two keyed: `find_rulemaking_documents` (Regulations.gov) and `verify_citation` (CourtListener; the case half of the currency check). GovInfo US Code text is reached via the keyless **link service** (`/link/uscode/{title}/{section}`), so the planned GovInfo `api.data.gov` key turned out to be unnecessary; the two keyed sources read `REGULATIONS_GOV_API_KEY` and `COURTLISTENER_API_KEY` respectively. The connector's `README.md` documents the evidence envelope and the known limits.
